...
public class Base64ImgReplacedElementFactory implements ReplacedElementFactory {
    ...
    protected FSImage buildImage(String srcAttr, UserAgentCallback uac) throws IOException,
        ...
        ClassPathResource classPathResource = new ClassPathResource("/image/" + srcAttr);
//        System.out.println("classPath:" + classPathResource);
        InputStream inputStream = classPathResource.getInputStream();
        byte[] bytes = new byte[inputStream.available()];
        inputStream.read(bytes);
        inputStream.close();
        fiImg = new ITextFSImage(Image.getInstance(bytes));
